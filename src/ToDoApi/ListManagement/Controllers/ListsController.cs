...
public class ListsController : ApiControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<ListResponse>>> Create([FromBody] CreateListRequest request)
    {
        try
        {
            ...
            var createdList = await _mediator.Send(new GetToDoListByIdQuery(listId, userId));
            if (createdList == null)
    ...
    /// <summary>
    /// Retrieves all todo lists for the authenticated user, with optional pagination.
    /// </summary>
    [HttpGet]
    ...
    public async Task<ActionResult<ApiResponse<PagedListResponse>>> GetAll([FromQuery] PaginationParameters parameters)
    {
        ...
        {
            var userId = GetCurrentUserId();
            var query = new GetAllToDoListsByUserQuery(userId);
    ...
    public async Task<ActionResult<ApiResponse<ListResponse>>> GetById(Guid id)
    {
        ...
        {
            var userId = GetCurrentUserId();
            var query = new GetToDoListByIdQuery(id, userId);
    ...
    public async Task<IActionResult> UpdateDescription(Guid id, [FromBody] UpdateListDescriptionRequest request)
    {
        ...
        {
            var userId = GetCurrentUserId();
            var command = new UpdateToDoListDescriptionCommand(id, userId, request.Description);
            var success = await _mediator.Send(command);
    ...
    public async Task<IActionResult> MarkAsCompleted(Guid id)
    {
        ...
        {
            var userId = GetCurrentUserId();
            var command = new MarkToDoListAsCompletedCommand(id, userId);
            var success = await _mediator.Send(command);
    ...
    public async Task<IActionResult> MarkAsIncomplete(Guid id)
    {
        ...
        {
            var userId = GetCurrentUserId();
            var command = new MarkToDoListAsIncompleteCommand(id, userId);
            var success = await _mediator.Send(command);
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        {
            var userId = GetCurrentUserId();
            var command = new DeleteToDoListCommand(id, userId);
            var success = await _mediator.Send(command);
